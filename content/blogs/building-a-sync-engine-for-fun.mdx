...
**Policy checks.** Sync is not allowed to become a side door around authorization. I did not want database-level RLS as the source of truth; I wanted the runtime to make the authorization decision once and apply it consistently to reads, writes, functions, and sync. That pushed the policy language toward a small expression grammar: `auth.userId != null`, `auth.userId == data.authorId`, role checks, boolean operators. It is not a general-purpose programming language, and that is the point. It should fail closed and be readable during an incident.
...
The realtime protocol sends per-row change events. The client maintains a local replica and React hooks derive the view from that replica. That is less magical than server-side live-query invalidation, but it made the first version much easier to reason about.
