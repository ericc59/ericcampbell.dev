...
I have somewhere around twenty side projects in various states of half-finished. Most of them need the same unglamorous substrate: durable state, an API, and a realtime path where two browsers can see the same update without a refresh. Every time I start a new one I end up reaching for Convex, or Postgres behind a Next.js API route with a Pusher channel attached.
