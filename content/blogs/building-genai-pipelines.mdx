...
At Flow Auctions, I built GenAI pipelines for generating lot descriptions, auto-pricing items, creating marketing emails, and producing listing images. They run in production.

Some notes from building them.

## Start With the Workflow, Not the Model

Teams often start with "let's use GPT-5" instead of "what's the workflow?"
...
Benchmarks don't settle this. Test each model on your own data and keep the one that works.
...
AI costs add up fast. A naive implementation of our lot description feature would cost $0.15 per item. At 10,000 items per month, that's $1,500 for descriptions alone.
...
Most teams reach for a RAG pipeline first. Index your documents, retrieve relevant chunks, stuff them in the context.

We tried this for coin grading guidelines. It didn't work well.

Coin grading combines visual assessment with encyclopedic knowledge. The model needs to know that a 1909-S VDB Lincoln cent in Good condition is worth 100x more than a 1909 Lincoln cent in Good condition. You can't retrieve that. The model has to already know it.
...
This is still retrieval, but we retrieve examples rather than knowledge chunks.

## Human-in-the-Loop

The biggest lesson is to make the human's job easier instead of removing them from the loop.
...
This feedback loop is more valuable than any benchmark. It runs on real users, real items, and real acceptance criteria.
...
The AI would decide how many steps to take based on what it sees. The workflow adapts to the input instead of being fixed in advance.
