...
At Flow Auctions, we use GenAI throughout the product — generating lot descriptions, auto-pricing items, creating marketing emails, producing listing images. These aren't demos. They run in production, processing thousands of items per week.

Some notes from the trenches.

## Start With the Workflow, Not the Model

The first mistake I see teams make: starting with "let's use GPT-4" instead of "what's the workflow?"
...
- **GPT-4o** for image analysis — slightly better at reading text in photos (mint marks, dates)
- **GPT-4o mini** for classification — fast and cheap for "is this a coin or a bill?"

This isn't about benchmarks. It's about testing each model on your actual data and seeing what works.

We also use fallbacks. If Claude is slow (happens during high load), we fall back to GPT-4. The prompts are slightly different, but the output schema is identical. Users don't notice.
...
We're building multi-step workflows where the AI plans its own execution. For a coin collection intake:
...
The AI decides how many steps to take based on what it sees. That's where this is heading — agents that adapt their workflow to the input.
