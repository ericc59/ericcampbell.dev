...
When I started building Stack0, I had a simple goal: make it easy to add email, file storage, video processing, and third-party integrations to any app. One import, one API key, done.

What I didn't anticipate was how many design decisions that would require.

## The Core Tension

Unified SDKs carry a built-in tension. Consistency says that a user who has called `stack0.cdn.upload()` should be able to guess how `stack0.email.send()` works. But the services really are different. Email has recipients and subjects. CDN has mime types and transformations. Forcing them into identical shapes creates awkward abstractions.
