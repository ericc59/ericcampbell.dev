...
summary: Agents can think but can't act. What we still need to build, and why approval workflows are what make agents trustworthy.
---

Most agent discussion is about capabilities. Can they reason? Can they plan? Can they use tools?

Those are mostly solved. Claude and GPT-5 reason through complex tasks, break problems into steps, and call APIs.

The infrastructure is missing. These are the unglamorous parts that let an agent do real work.
...
The demo assumes the agent should act on its own. Production needs gates, approvals, and audit trails.
...
You need this for debugging, for compliance, and for answering "what happened at 3am last Tuesday?"
...
This is the infrastructure gap, and it separates demos from production.

## What Needs to Exist

Someone needs to build this infrastructure. Stripe did it for payments and Twilio did it for messaging. Agent deployment needs the same treatment.
...
Deploying a production agent should be as easy as deploying a production API.
